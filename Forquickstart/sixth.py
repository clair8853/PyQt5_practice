...
# 파일 경로 준비를 위한 경량 작업을 처리하는 Worker 클래스
class FilePathWorker(QThread):
...
class MainWindow(QMainWindow):
    ...
    def open_file(self):
        ...
        if file_name:
            self.start_file_path_worker(file_name)

    def start_file_path_worker(self, file_name):
        self.statusbar.showMessage('Preparing to load data...')

        # Worker 스레드를 시작하여 파일 경로 준비 (실제 로딩은 메인 스레드에서)
        self.worker = FilePathWorker(file_name)
